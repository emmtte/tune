...
import fns from 'date-fns' ; const { add, addSeconds, parseISO, getUnixTime, format} = fns
...
console.log('now:',now) ; console.log('end:',end)                                                                                                                                                                  let cover=obj.CoverLink.replace('cover','cover/500')
let caption= `<b>${decode(obj.Album)}</b>\n<i>${decode(obj.Artist)}</i>\n${decode(obj.Track)} [${length}]`
if (obj.Album !== 'StationID') {bot.api.sendPhoto(chatId, cover, {caption:caption, parse_mode:'html'})}
